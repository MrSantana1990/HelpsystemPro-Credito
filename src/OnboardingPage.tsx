  ...
  return <main className="onboarding-page"><section className="onboarding-card">
    ...
    {error && !info ? <div className="form-error onboarding-error">{error}</div> : <>
      ...
      <form className="dialog-form onboarding-form" onSubmit={submit}>
        ...
        {incomeType === "clt" && <div className="income-specific"><strong>Informações do vínculo CLT</strong><label>Empresa empregadora<input name="employerName" required maxLength={160} /></label><div className="form-row"><label>Data de admissão<input name="employmentStartDate" type="date" required /></label><label>Competência do holerite<input name="incomeReferenceMonth" type="month" required /></label></div></div>}
        {incomeType === "autonomo" && <div className="income-specific"><strong>Informações da atividade autônoma</strong><label>Atividade exercida<input name="activityDescription" required maxLength={160} /></label><div className="form-row"><label>Início da atividade<input name="activityStartDate" type="date" required /></label><label>Mês do comprovante/extrato<input name="incomeReferenceMonth" type="month" required /></label></div></div>}
        {incomeType === "beneficio" && <div className="income-specific"><strong>Informações do benefício</strong><div className="form-row"><label>Tipo do benefício<input name="benefitType" required maxLength={120} /></label><label>Número do benefício<input name="benefitNumber" required maxLength={60} /></label></div><label>Mês do extrato<input name="incomeReferenceMonth" type="month" required /></label></div>}
        {incomeType === "empresario" && <div className="income-specific"><strong>Informações do MEI ou empresa</strong><label>Nome empresarial ou nome fantasia<input name="businessName" required maxLength={160} /></label><div className="form-row"><label>CNPJ<input name="businessDocument" required inputMode="numeric" maxLength={18} /></label><label>Início da atividade<input name="businessStartDate" type="date" required /></label></div><label>Mês do comprovante de faturamento<input name="incomeReferenceMonth" type="month" required /></label></div>}
        {incomeType === "outro" && <div className="income-specific"><strong>Detalhamento da renda</strong><label>Explique a origem da renda<input name="incomeDescription" required maxLength={160} /></label><label>Mês do comprovante<input name="incomeReferenceMonth" type="month" required /></label></div>}
